...
export default({ handleSearch, searchParam }) => {
  ...
  const loadMonths = () => {
    setIsLoading(true)
    callGetApiWithAuth('news/months', onGetMonths, ()=>setIsLoading(false))
  }
    ...
    <List component='nav' className={classes.listSection}>
      {isLoading? [...Array(6).keys()].map(index => 
        ...
        >
          ...
          />
          <ListItemSecondaryAction>
            <IconButton edge='end' className={classes.newsCount} style={{backgroundColor: searchParam.month==month.value?'#5d487a':'#ADB0C2'}}>
              {month.count}
            </IconButton>
          </ListItemSecondaryAction>
        </ListItem>
