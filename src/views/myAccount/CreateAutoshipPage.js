...
import ProductsGrid from './createAutoship/ProductsGrid'
import CartSummary from './createAutoship/CartSummary'
import MakeAutoshipModal from './createAutoship/MakeAutoshipModal'
import { callPostApiWithAuth } from 'utils/api'

export default function CreateAutoshipPage() {
  ...
  const handleAutoship = (autoshipData) => {
    let orderDetails = []
    cartItems.map(item => {
      orderDetails.push({
    ...
    let data = {
      autoship: {
        day_of_month: autoshipData['day_of_month'],
      },
      autoshipDetails: orderDetails,
    }
    setIsSubmiting(true)
    callPostApiWithAuth(`autoships`, data, onDoneAutoship, onFailAutoship)
  }
  const onDoneAutoship = () => {
    setIsSubmiting(false)
    setIsOpenedCreate(false)
    setSuccessMessage('Autoship created successfully')
    setCartItems([])
    setTimeout(() => {
