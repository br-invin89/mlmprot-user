...
export default function ProductCard ({ data, openEdition, afterSuccessUpdate, onError }) {
  ...
  const onDoneCancel = () => {
    ...
    afterSuccessUpdate("Subscription cancelled.");
  };
  const onFailCancel = () => {
    setIsCanceling(false);
  };
  ...
  useEffect(() => {
    ...
    data.details.forEach(el=>{
      pcAmount += el.product.pc_amount*el.quantity
      scAmount += el.product.sc_amount*el.quantity
    })
    setPcAmount(pcAmount)
    setScAmount(scAmount)
  }, [myUser, data])
